...
public class puzzle
{
	...
	class heap
	{
		...
		public nod delete()
		{
			...
			/*while(2*k+1<=N && (Heap[2*k].price<Heap[k].price||Heap[2*k+1].price<Heap[k].price))
			{
				...
				else if (Heap[2*k+1].price<Heap[2*k].price)
				{
					nod tmp = Heap[2*k+1];
					Heap[2*k+1]=Heap[k];
					Heap[k]= tmp;
				...
				{
					if (Heap[2*k+1].mov>Heap[2*k].mov)
					{
						nod tmp = Heap[2*k];
						Heap[2*k]=Heap[k];
						Heap[k]= tmp;
	...
	public HashMap<String,Vertice> start_graph(puzzle new_puz,Vector<String> graph_ver)
	{
	...
	public static void main(String args[])
	{
		try
		{
			...
			while(no_of_words>0)
			{
				...
				/*Vertice a =  hm.get("12346G758");
				for (int k = 0 ; k<4;k++)
				{
					System.out.println(a.edges[k]);
				}*/
				new_puz.dijkstra(hm,"12346875G","12346857G",costs);
				no_of_words--;
			}
		...
		//System.out.println(graph_ver.size());
	}
	...
	public  void dijkstra(HashMap<String,Vertice> hm,String source,String dest,int[] costs)
	{
		...
		{
		    String tmp = it.next();
			put_in_graph(pm,tmp,costs);
		} 
		pm.get(source).distance = 0;*/
		...
		{	
			...
			if (ed.edges[0]!=null)
			{
				/*if (min.u==1)
				{
					nod up = new nod(ed.edges[0],min.prev.mov+1,costs[ed.edge_cost[0]]+min.price,min);
					up.prev = min;
					up.d=0;
					up.prev_reached_by = "D";
				...
				if (pm.get(ed.edges[0])==null &&found.get(ed.edges[0])==null)
				{
			...
			if (ed.edges[1]!=null)
			{
				/*if (min.d==1)
				{
					...
					nod down = new nod(ed.edges[1],min.prev.mov+1,costs[ed.edge_cost[1]]+min.price,min);
					down.prev = min;
					down.u=0;
					down.prev_reached_by = "U";
					down.moved = ed.edge_cost[1];
					if (pm.get(down.s)==null)
					{	
						Heap.insert(down);
						pm.put(edi.edges[1],down);
					}
			...
			if (ed.edges[2]!=null)
			{
				/*if (min.l==1)
				{
					nod left = new nod(ed.edges[2],min.prev.mov+1,costs[ed.edge_cost[2]]+min.price,min);
					left.prev = min;
					left.r=0;
					left.prev_reached_by = "R";
					left.moved = ed.edge_cost[2];
					if (pm.get(left.s)==null)
					{	
						Heap.insert(left);
						pm.put(edi.edges[2],left);
					}
			...
			if (ed.edges[3]!=null)
			{
				/*if (min.r==1)
				{
					nod right = new nod(ed.edges[3],min.prev.mov+1,costs[ed.edge_cost[3]]+min.price,min);
					right.prev = min;
					right.l=0;
					right.prev_reached_by = "L";
					right.moved = ed.edge_cost[3];
					if (pm.get(right.s)==null)	
					{
						Heap.insert(right);
						pm.put(edi.edges[3],right);
					}
		...
		{
			System.out.println(min.mov + " "+min.price);
			...
			System.out.println(to_print.toString());
		}
		else
		{
